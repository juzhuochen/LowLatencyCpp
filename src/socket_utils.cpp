...
auto setNoDelay(int fd) -> bool {
    ...
    return (
        setsockopt(
            fd, IPPROTO_TCP, TCP_NODELAY, reinterpret_cast<void *>(&one),
            sizeof(one))
        != -1);
}
...
auto setTTL(int fd, int ttl) -> bool {
    return (
        setsockopt(
            fd, IPPROTO_IP, IP_TTL, reinterpret_cast<void *>(&ttl), sizeof(ttl))
        != -1);
}
auto setMcastTTL(int fd, int mcast_ttl) -> bool {
    return (
        setsockopt(
            ...
            reinterpret_cast<void *>(&mcast_ttl), sizeof(mcast_ttl))
        != -1);
}
auto setSOtimestamp(int fd) -> bool {
    ...
    return (
        setsockopt(
            fd, SOL_SOCKET, SO_TIMESTAMP, reinterpret_cast<void *>(&one),
            sizeof(one))
        != -1);
}

[[nodiscard]]inline auto creatSocket(
    Logger &logger,
    ...
    std::string time_str;
    const auto ip = t_ip.empty() ? getIfaceIP(t_ip) : t_ip;
    logger.log(
        "%:% %() % ip:% iface:% port:% is_udp:% is_blocking:% is_listening:% ttl:% SO_time:% \n",
        __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str),
        ip, iface, port, is_udp, is_blocking, is_listening, ttl,
        need_to_timestamp);
    addrinfo hints{};
    ...
    if (rc) {
        logger.log(
            "getaddrinfo() failed. error:% errno:%\n", gai_strerror(rc),
            strerror(errno));
        return -1;
    }
    int socket_fd = -1;
    ...
    for (addrinfo *rp = result; rp; rp = rp->ai_next) {
        ...
        if (socket_fd == -1) logger.log("socket() failed. errno:%\n", strerror(errno));
        return -1;
        if (!is_blocking){
            if(!setNonBlocking(socket_fd)){
                logger.log("setNonblcking() failed. errno:%",strerror(errno));
            }
        }
    }
...
int main() {
    std::cout << Common::getIfaceIP("wlp0s20f3") << std::endl;
}