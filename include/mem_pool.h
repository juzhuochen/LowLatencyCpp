...
class MemPool final {
  ...
  public:
    explicit MemPool(std::size_t num_elems) : m_store(num_elems, {T(), true}) {
        ASSERT(
            reinterpret_cast<const ObjectBlock *>(&(m_store[0].m_object))
                == (m_store).data(),
    ...
    T *allocate(Args... args) noexcept {
        ...
        ASSERT(
            obj_block->m_is_free, "Expected free OnjectBlock at index: "
    ...
    auto deallocate(const T *elem) noexcept {
        ...
        ASSERT(
            elem_idx >= 0 && static_cast<size_t>(elem_idx) < m_store.size(),
            "Element being deallocted does not belong to this memory pool.");
        ASSERT(
            !m_store[elem_idx].m_is_free,
